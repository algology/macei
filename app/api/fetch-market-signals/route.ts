...
export async function POST(request: NextRequest) {
  console.log("--- [fetch-market-signals] Received Request ---");
  let requestBody;
  try {
    requestBody = await request.json();
    console.log("[fetch-market-signals] Parsed Request Body:", JSON.stringify(requestBody, null, 2));

    if (requestBody.search_context && requestBody.context_type === 'hypotheses') {
        console.log("[fetch-market-signals] SUCCESS: Received context_type='hypotheses'. Context length:", requestBody.search_context.length);
    } else {
        console.log("[fetch-market-signals] NOTE: Hypothesis context NOT received or context_type is not 'hypotheses'.");
        console.log("[fetch-market-signals] Received context_type:", requestBody.context_type);
    }

  } catch (e) {
    console.error("[fetch-market-signals] Error parsing request body:", e);
    try {
        const textBody = await request.text();
        console.log("[fetch-market-signals] Raw Request Body (Text):", textBody);
    } catch (textError) {
        console.error("[fetch-market-signals] Error reading request body as text:", textError);
    }
  ...
  console.log("[fetch-market-signals] Destructured Values:", { ideaName, category, missionName, organizationName, context_type, search_context_length: search_context?.length });
  ...
  // Determine the primary context for the prompt
  const primary_context_for_prompt = context_type === 'hypotheses' && search_context ? search_context : signals || "";
  const context_label = context_type === 'hypotheses' ? "Hypotheses / Key Questions" : "Market Signals (Keywords)";
  ...
  Mission: ${missionName || ""}
  ${context_label}: ${primary_context_for_prompt}
  AI Analysis Summary (if available): ${aiAnalysis || "N/A"}
  
  Based on the idea and the provided context (${context_label}), create SPECIFIC and TARGETED search queries that will yield highly relevant results. Focus on the core concepts and avoid generic terms.

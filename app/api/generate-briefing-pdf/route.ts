...
export async function POST(request: NextRequest) {
  try {
    ...
    const htmlContent = generateHtml(briefing);

    // 6. Generate PDF using Puppeteer
    // Note: For production deployment (esp. on serverless platforms like Vercel),
    // you may need to use a different approach or configuration for Puppeteer
    // See: https://vercel.com/guides/using-puppeteer-with-vercel
    const browser = await puppeteer.launch({
      // In production, especially on serverless, you'd likely need to use
      // different args like --no-sandbox for compatibility
...
function generateHtml(briefing: any) {
  // Extract idea name
